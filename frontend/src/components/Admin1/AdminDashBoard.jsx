...
const AdminDashBoard = ({ title }) => {
  ...
  const compareDates = (date) => {
    ...
    if (date.getYear() != today.getYear())
      return date.getYear() < today.getYear() ? -1 : 1;
    if (date.getMonth() != today.getMonth())
      return date.getMonth() < today.getMonth() ? -1 : 1;
    if (date.getDate() != today.getDate())
      return date.getDate() < today.getDate() ? -1 : 1;
    return 0;
  };
    ...
    <div className="flex-grow">
      ...
      <div className="p-12 mt-2">
        <div className="flex py-10 justify-end">
          <span
            className="inline-flex bg-hex-blue p-2 text-hex-lightgrey"
            onClick={() => setShowMenteeBox(true)}
          >
            Add Mentee
          </span>
        </div>
        ...
        <table className="w-full border-none text-left text-[#19105B]">
          ...
          {dateRanges.map((date) => (
            <tr className="bg-[#F2F1F6] border-b border-[#D1CFDE]">
              ...
              <td className="p-2 pl-2">
                <div
                  className={`rounded-lg p-1 px-2 inline-flex ${
                    date.status ? "bg-[#0dff09b8]" : "bg-[#FF095C]"
                  }`}
                >
